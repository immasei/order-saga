...
public class StockController {
    ...
    @PostMapping
    public ResponseEntity<StockDTO> assignStock(@RequestBody @Valid AssignStockDTO stockDto) {
        StockDTO stock = warehouseService.assignStock(stockDto); // upsert

        return ResponseEntity.ok(stock);
    }

    // Assign stocks in batch (can be to different warehouse)
    @PostMapping("/batch")
    public ResponseEntity<List<StockDTO>> assignStocks(@RequestBody @Valid List<AssignStockDTO> stockDtos) {
        List<StockDTO> stocks = warehouseService.assignStocksInBatch(stockDtos); // upsert
        return ResponseEntity.ok(stocks);
    }

    @GetMapping("/{productCode}")
    public ResponseEntity<List<StockDTO>> getStocksByProduct(@PathVariable String productCode) {
        List<StockDTO> stocks = warehouseService.getStocksByProductCode(productCode); // upsert
        return ResponseEntity.ok(stocks);
    }
